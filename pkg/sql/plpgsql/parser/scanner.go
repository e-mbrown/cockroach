...
func (s *Scanner) Scan(lval ScanSymType) {
	...
	default:
		if plpgsqllexbase.IsDigit(ch) {
			s.scanNumber(lval, ch)
			return
		}
		if plpgsqllexbase.IsIdentStart(ch) {
			s.scanIdent(lval)
			return
		}
	}
...
func (s *Scanner) scanIdent(lval ScanSymType) {
	...
	for {
		...
		if !plpgsqllexbase.IsIdentMiddle(ch) {
			break
		}
	...
	if isLower && isASCII {
		...
		lval.SetStr(plpgsqllexbase.NormalizeName(s.in[start:s.pos]))
	}
...
func (s *Scanner) scanNumber(lval ScanSymType, ch int) {
	...
	for {
		ch := s.peek()
		if (isHex && plpgsqllexbase.IsHexDigit(ch)) || plpgsqllexbase.IsDigit(ch) {
			s.pos++
			continue
		}
		if ch == 'x' || ch == 'X' {
...
outer:
	...
	if ch == identQuote {
		lval.SetStr(plpgsqllexbase.NormalizeString(s.finishString(buf)))
	} else {
		lval.SetStr(s.finishString(buf))
	}
	return true
...
outer:
	for {
		...
		default:
			...
			if !foundStartTag && !plpgsqllexbase.IsIdentStart(ch) && !plpgsqllexbase.IsDigit(ch) {
				return false
			}
			s.pos++
			if possibleEndTag {
