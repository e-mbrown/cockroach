...
func registerJasyncSQLMulti(r registry.Registry) {
	clusters := []spec.ClusterSpec{
		r.MakeClusterSpec(1, spec.ReuseNone()),
		r.MakeClusterSpec(1, spec.ReuseNone()),
	}

	runJasyncSQLMulti := func(ctx context.Context, t test.Test, c cluster.Cluster, c2 cluster.Cluster) {
		if c.IsLocal() {
			t.Fatal("can not be run in local mode")
		}
		node := c.Node(1)
		...
		t.Status("setting up cockroach")
		c.Put(ctx, t.Cockroach(), "./cockroach", c.All())
		c2.Put(ctx, t.Cockroach(), "./cockroach", c2.All())

		c.Start(ctx, t.L(), option.DefaultStartOpts(), install.MakeClusterSettings(), c.All())
		c2.Start(ctx, t.L(), option.DefaultStartOpts(), install.MakeClusterSettings(), c2.All())

		version, err := fetchCockroachVersion(ctx, t.L(), c, node[0])
		if err != nil {
			t.Fatal(err)
		}
		_, err = fetchCockroachVersion(ctx, t.L(), c, node2[0])
		if err != nil {
			t.Fatal(err)
		}
		...
		if err := alterZoneConfigAndClusterSettings(ctx, t, version, c, node2[0]); err != nil {
			t.Fatal(err)
		}
		...
		if err := c2.RunE(ctx, node, fmt.Sprintf("cd /mnt/data1/jasync-sql && git checkout %s",
			supportedJasyncCommit)); err != nil {
			t.Fatal(err)
		}
		...
		_ = c.RunE(ctx, node, `mkdir -p ~/logs/report/jasyncsql-results`)
		_ = c2.RunE(ctx, node, `mkdir -p ~/logs/report/jasyncsql-results`)
		...
		result2, err := repeatRunWithDetailsSingleNode(
			ctx,
			c,
			t,
			node,
		...
		parseAndSummarizeJavaORMTestsResults(
			ctx, t, c2, node, "jasyncsql", []byte(result.Stdout),
			blocklistName, expectedFailures, ignorelist, version, supportedJasyncCommit)
	}
	r.Add(registry.TestSpec{
		Name:         "jasyncMulti",
